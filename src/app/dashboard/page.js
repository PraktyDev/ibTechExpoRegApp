...
async function getData() {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/attendees`, { cache: "no-store" })
      return res.data;
    } catch (error) {
  ...
  return (
    <div className="flex flex-col tablet:flex-row m-auto gap-16 tablet:gap-28 laptop:gap-40">
        ...
        <Card className="relative flex flex-col items-center justify-center w-56 h-56 hover:border-2 hover:border-b-blue-600 hover:bg-yellow-50 hover:duration-500 transition-all">
            <p className="absolute right-3 top-3 flex items-center justify-center bg-blue-800 text-white font-bold p-2 w-auto h-7 rounded-full">{data.length}</p>
            <CardContent className="flex flex-col gap-4 items-center justify-center">
